...
interface ContactLimitUpsellModalProps {
  open: boolean
...
export function ContactLimitUpsellModal({
  open,
  onOpenChange,
  used,
  limit,
  ...
  premiumContactLimit,
}: ContactLimitUpsellModalProps) {
  const [dismissed, setDismissed] = useState(false)

  function handleDismiss() {
    setDismissed(true)
    onOpenChange(false)
    captureClientEvent('contact_limit_upsell_dismissed', {
      used,
      limit,
  ...
  function handleUpgradeClick() {
    captureClientEvent('contact_limit_upsell_clicked', {
      used,
      limit,
      premium_monthly_price: premiumMonthlyPrice,
      ...
      <DialogContent className="sm:max-w-md" showCloseButton={false}>
        <DialogHeader className="text-center">
          ...
          <DialogDescription className="text-forest-mid">
            Free members can start {limit} conversations per month. You&apos;ve used {used}.
            Upgrade to Premium to unlock more trade opportunities.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="rounded-lg border bg-card p-4 space-y-3">
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li className="flex items-start gap-2">
                ...
                <span>{premiumContactLimit} contacts per month ({premiumContactLimit / limit}x more)</span>
              </li>
              <li className="flex items-start gap-2">
                <Check className="h-4 w-4 text-green-500 shrink-0 mt-0.5" />
